<template>
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
        <template v-slot:activator="{ on }">
            <v-btn v-on="on" depressed rounded style="background-color: #3b5998;color: white">Create Admin</v-btn>
        </template>
        <v-card>
            <v-card-title>
                <span style="color: #3b5998" class="headline">Create Admin</span>
            </v-card-title>
            <v-card-text>
                <v-container>
...
<script>
    export default {
        ...
        methods:
            {
                signup() {
                    axios.post('/lms/api/create/admin', this.form)
                        .then(res => this.dialog = false,
                            EventBus.$emit('newAdmin',this.form),
                            this.$toasted.show('Admin Created',{type:'success'}),
                            this.x=0,
                        )
