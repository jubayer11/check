<template>
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
        <template v-slot:activator="{ on }">
            <v-btn v-on="on" depressed rounded style="background-color: #3b5998;color: white">Create Student</v-btn>
        </template>
        <v-card>
            <v-card-title>
...
<script>
    export default {
        ...
        methods:
            {
                signup() {
                    axios.post('/lms/api/create/student', this.form)
                        .then(res =>
                            this.dialog = false,
                            EventBus.$emit('newStudent',this.form),
                            this.$toasted.show('Student Created',{type:'success'}),
                            this.form={},
